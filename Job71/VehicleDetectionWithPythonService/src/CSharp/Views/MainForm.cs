...
namespace VehicleDetection_8._0_
{
    public partial class MainForm : Form
    {
        ...
        private string _modelSegmentPath;
        private string _imageProcessedPath;
        #endregion
        ...
        private void InitializePaths()
        {
            ...
            _modelSegmentPath = Path.Combine(_rootDir, "model", "yolov8n-seg.pt");
            _imageProcessedPath = Path.Combine(_rootDir, "resources", "Image", "Processed", "img5_mask.jpg");
        }
        #endregion
        ...
        private async Task ProcessImage()
        {
            ...
            try
            {
                ...
                foreach (string file in allFiles)
                {
                    ...
                    var (results, density) = await gRpc(file);
                    ...
                    Invoke(new Action(() =>
                            {
                                ...
                                pictureBox1.Refresh();
                                lbPercent.Text = $"Mật độ phương tiện chiếm dụng: {Math.Round(density, 2)}%";
        ...
        private async Task<(List<dynamic>, float)> gRpc(string imagePath)
        {
            try
            {
                // Địa chỉ server gRPC
                ...
                var request2 = new DensityRequest
                {
                    ModelPath = _modelSegmentPath,
                    PathImageProcessed = _imageProcessedPath,
                    PathImage = imagePath
                };

                var response2 = await client.CalculateDensityAsync(request2);

                // Xử lý phản hồi
                if (response2.Success)
                {
                    Console.WriteLine("Message: " + response2.Message);
                    Console.WriteLine("Density: " + response2.Density);
                    ...
                    return (detections, response2.Density);
                }
                else
                {
                    return (new List<dynamic>(), 0); // Trả về danh sách rỗng nếu có lỗi
                }
            ...
            catch (Exception ex)
            {
                ...
                return (new List<dynamic>(), 0); // Trả về danh sách rỗng nếu có ngoại lệ
            }
        }
