...
Original file is located at
    https://colab.research.google.com/drive/12IsLSnECHzrKO7AXWpkF58na9k5QrStO
"""
...
"""# 1. Phần tính diện tích phương tiện chiếm dụng lòng đường"""

def calculate_road_occupancy(original_frame, processed_frame, model):
    # Chuyển ảnh lòng đường thành ảnh nhị phân
    gray_road_mask = cv2.cvtColor(processed_frame, cv2.COLOR_BGR2GRAY)
    ...
    for i in range(len(results[0].masks)):
        ...
        if names[class_id] in vehicle_list:
            ...
            vehicle_area = np.count_nonzero(mask.cpu().numpy())
            # vehicle_area = np.count_nonzero(mask.cpu().numpy())
            sum_vehicle_area += vehicle_area
            num_vehicles += 1

    # Tính phần trăm chiếm dụng
    percentage_occupancy = (sum_vehicle_area / road_area) * 100 if road_area > 0 else 0

    return {
        "road_area": road_area,
        "num_vehicles": num_vehicles,
        "sum_vehicle_area": sum_vehicle_area,
        "percentage_occupancy": percentage_occupancy
    }

# Ví dụ sử dụng
model = YOLO("yolov8s-seg.pt")
original_frame = cv2.imread("/content/drive/MyDrive/NGANGIANGINTERNSHIP/Job69/img7.jpg")
processed_frame = cv2.imread("/content/drive/MyDrive/NGANGIANGINTERNSHIP/Job69/img5_mask2.jpg")

# result = calculate_road_occupancy(original_frame, processed_frame, model)
# print(result)
...
class SpeedDetector:
    ...
    def process_video(self, video_path, output_path='output_video.mp4', lane_width_pixels=100):
        ...
        while cap.isOpened():
            ...
            for result in results:
                ...
                for box in boxes:
                    ...
                    if class_id in self.vehicle_classes:
                        ...
                        # Tính toán trung tâm phương tiện
            ...
            result = calculate_road_occupancy(frame, processed_frame, model)
            print(result)

            # Hiển thị thống kê phương tiện trên khung hình
            y_offset += 25
            cv2.putText(frame, f"Ty le chiem dung long duong: {result['percentage_occupancy']:.2f}%",
                       (10, y_offset),
...
def main():
    ...
    video_path = '/content/drive/MyDrive/NGANGIANGINTERNSHIP/Job69/b.mp4'
