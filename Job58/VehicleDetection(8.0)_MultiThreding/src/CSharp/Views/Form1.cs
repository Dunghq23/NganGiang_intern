...
namespace VehicleDetection_8._0_
{
    public partial class Form1 : Form
    {
        ...
        Dictionary<string, DetectionResult> frameTimeExecute = new Dictionary<string, DetectionResult>();
        ...
        private void ExtractAndDetectImages(string videoPath)
        {
            try
            {
                    ...
                    while (frameCount < totalFrames)
                    {
                        using (Mat frame = new Mat())
                        {
                            ...
                            if (frameCount % _frameSkipQuantity == 0)
                            {
                                ...
                                frameTimeExecute.Add($"frame_{frameCount}", results);
                ...
                string logPath = Path.Combine(_rootDir, "resources", "Logs", "TimeExecute.txt");
                ExportDictionaryToFile(frameTimeExecute, logPath);
            }
            catch (Exception ex)
            {
