...
while cap.isOpened():
    ...
    # Đếm số phương tiện trong khung hình
    total_vehicles = len(results)  # Số lượng bounding box trong frame hiện tại

    # Vẽ bounding box lên khung hình kết quả
    annotated_frame = resized_frame.copy()
    for result in results:
        annotated_frame = result.plot()  # Vẽ bounding box và nhãn lên khung hình

    # Hiển thị tổng số phương tiện trên khung hình
    cv2.putText(annotated_frame, f'Total Vehicles: {total_vehicles}', (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)

    # Vẽ đường kẻ ngang 1/3 màn hình từ dưới lên (nếu cần)
    line_y_position = int(new_height * 2 / 3)
