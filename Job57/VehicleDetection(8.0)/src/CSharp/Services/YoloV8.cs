...
namespace VehicleDetection.src.CSharp.Services
{
    public class YoloV8
    {
        ...
        Dictionary<string, int> objectKeyValue = new Dictionary<string, int>();
        ...
        public Dictionary<string, int> Detect(string imagePath, string outputImagePath)
        {
            objectKeyValue.Clear();
        ...
        private void DrawBoundingBoxes(string imagePath, List<Rectangle> boxes, List<float> scores, List<int> classIds, string outputImagePath)
        {
            ...
            for (int i = 0; i < boxes.Count; i++)
            {
                ...
                if (objectKeyValue.ContainsKey(className))
                {
                    objectKeyValue[className] += 1;
                }
                else
                {
                    objectKeyValue.Add(className, 1);
                }
